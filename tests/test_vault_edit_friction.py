...
from obsidian_vault_mcp.models import VaultEditOperationInput, VaultMutationContextInput
...
def _governed_server_edit(vault_dir):
    """Create a LifeOS-governed edit target and its required mutation preflight."""
    path = "01-input/test-note.md"
    target = vault_dir / path
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text((vault_dir / "test-note.md").read_text(), encoding="utf-8")
    digest = hashlib.sha256(target.read_bytes()).hexdigest()
    mutation = VaultMutationContextInput.model_validate({
        "reason": "upstream vault_edit compatibility test",
        "destination": path,
        "preflight": {
            "source_input_class": "test-fixture",
            "entity_area": "01-input",
            "capability": "capture",
            ...
            "operation": "update",
            "confidence": 0.99,
            "reason": "test fixture resolves one governed destination",
            "preimage_requirement": f"sha256:{digest}",
            "rollback_target": f"restore-preimage:{path}",
        },
    })
    return path, target, mutation
...
def test_server_vault_edit_accepts_old_new_aliases(vault_dir):
    ...
    path, target, mutation = _governed_server_edit(vault_dir)
    ...
    assert "error" not in result
    assert result["changed"] is True
    assert "more focused content" in target.read_text()
...
def test_server_vault_edit_rejects_empty_old_text(vault_dir):
    ...
    before = target.read_text()
    with pytest.raises(ValidationError):
        server.vault_edit(path, [{"old_text": "", "new_text": "x"}], mutation)
    assert target.read_text() == before
